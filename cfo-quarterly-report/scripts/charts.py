...
def main(computed_path, outdir):
    ...
    ax.yaxis.set_major_formatter(FuncFormatter(usd)); ax.set_ylabel("Revenue (USD, quarter)",fontsize=9.5)
    ...
    ax.set_ylim(0,ymax)
    legend=[Patch(color=NAVY,label="Actual"),Patch(color=GREEN,label="Record quarter"),
            Patch(color=ORANGE,label="Focal quarter")]
    ...
    fig,ax=plt.subplots(figsize=(9.2,3.6)); y=np.arange(len(tc))[::-1]
    ax.barh(y,cv,color=ccols,height=0.66,zorder=3); ax.set_yticks(y); ax.set_yticklabels(names,fontsize=9)
    ax.xaxis.set_major_formatter(FuncFormatter(usd)); ax.set_axisbelow(True); ax.spines[["top","right"]].set_visible(False)
    for yi,v in zip(y,cv): ax.text(v+max(cv)*0.012,yi,f"${v:,.0f}",va="center",fontsize=8,color="#333")
